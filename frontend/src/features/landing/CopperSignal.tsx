...
export function CopperSignal({ progress }: { progress?: MotionValue<number> }) {
  ...
  return <figure className="cm-copper-signal" aria-label="Copper at the center of market context, news intelligence and quantitative forecasts">
    ...
    <div className="cm-signal-trace">
      <div><span>03 / FORECAST</span><strong>Keep the range in view.</strong></div>
      <svg viewBox="0 0 300 68" aria-hidden="true">
        <motion.path d={linePath(previewSeries, 192, 62)} fill="none" stroke="var(--cm-copper)" strokeWidth="1.8" style={progress ? { pathLength: progress } : undefined}/>
        <path d="M192 10L240 3L295 0L295 52L240 32Z" fill="var(--cm-forecast)" opacity=".16"/>
        <path d="M192 10L240 15L295 20" fill="none" stroke="var(--cm-forecast)" strokeWidth="1.8" strokeDasharray="3 4"/>
      </svg>
    </div>
    <figcaption>Illustrative preview <span>Price path & possible range · sample data</span></figcaption>
  </figure>;
}
